...
def process_grid_string(grid_str, rows=33, cols=36):
    """Process grid string: keep only '@' as 1, everything else as 0, remove first/last 2 columns."""
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if idx < len(grid_str):
                char = grid_str[idx]
                if char == '@':  # Only '@' becomes 1
                    grid_array[i, j] = 1.0
                elif char == 'e':
                    grid_array[i, j] = -1.0
                # Everything else ('.', 'e', 'w') becomes 0
    
    # Remove first 2 and last 2 columns
    grid_array = grid_array[:, 2:-2]  # Shape: (33, 32)
...
def prepare_unified_data():
    """Prepare data with guaranteed consistent splitting."""
    print("Preparing unified data with consistent splitting...")
    ...
    for grid_id in sorted(grids.keys()):
        ...
        # Process grid string
        processed_grid = process_grid_string(grid_str)
...
def create_consistent_splits(processed_grids, heatmaps, grid_ids, throughputs):
    """Create consistent train/test splits that can be reused across models."""
    print("Creating consistent train/test splits...")
    ...
    # Split data
    train_grids = processed_grids[train_indices]
    ...
    test_grids = processed_grids[test_indices]
    ...
    print(f"Training set: {len(train_grids)} samples ({len(train_grids)/len(processed_grids)*100:.1f}%)")
    print(f"  Training throughput: min={train_throughputs.min():.3f}, max={train_throughputs.max():.3f}, mean={train_throughputs.mean():.3f}")
    print(f"Test set: {len(test_grids)} samples ({len(test_grids)/len(processed_grids)*100:.1f}%)")
...
def save_splits(splits, output_dir='unified_data_splits'):
    """Save the consistent data splits for reuse across models."""
    os.makedirs(output_dir, exist_ok=True)
    
    # Save splits as pickle files
    splits_file = os.path.join(output_dir, 'data_splits.pkl')
    with open(splits_file, 'wb') as f:
        pickle.dump(splits, f)
    
    # Save as numpy files for easy loading
    np.save(os.path.join(output_dir, 'train_grids.npy'), splits['train']['grids'])
    ...
    np.save(os.path.join(output_dir, 'test_grids.npy'), splits['test']['grids'])
...
def main():
    """Main function to prepare and save unified data splits."""
    print("=" * 60)
    print("UNIFIED DATA PREPARATION")
    print("=" * 60)
    
    # Prepare data
    processed_grids, heatmaps, grid_ids, throughputs = prepare_unified_data()
    
    # Create consistent splits
    splits = create_consistent_splits(processed_grids, heatmaps, grid_ids, throughputs)
    
    # Save splits for reuse
    output_dir = save_splits(splits)
    ...
    print("import pickle")
    print("with open('unified_data_splits/data_splits.pkl', 'rb') as f:")
    print("    splits = pickle.load(f)")
    print("\nOr load individual numpy files:")
    print("train_grids = np.load('unified_data_splits/train_grids.npy')")
    print("train_heatmaps = np.load('unified_data_splits/train_heatmaps.npy')")
    print("train_throughputs = np.load('unified_data_splits/train_throughputs.npy')")
